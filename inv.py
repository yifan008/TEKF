...
class INV_EKF():
    def __init__(self, robot_system, dataset):
        ...
        self.trans = np.identity(3*NUM_ROBOTS)

        for r in range(NUM_ROBOTS):
          self.trans[3*r:3*r+2, 3*r+2] = - self.J @ self.robot_system.xyt[3*r:3*r+2]

        self.cov_z = self.trans @ self.robot_system.cov @ self.trans.T

    def prediction(self, t):
        dataset = self.dataset

        # self.trans = np.identity(3*NUM_ROBOTS)
        # G = np.identity(3*NUM_ROBOTS)
    ...
    def relative_observation(self, t):
        ...
        if MType == 'pos':
          ...
          for rr in range(NUM_ROBOTS):
            self.robot_system.xyt[3*rr:3*rr+2] = rot_mtx(_dz[3*rr+2]) @ self.robot_system.xyt[3*rr:3*rr+2] + self.Bc(_dz[3*rr+2]) @ _dz[3*rr:3*rr+2]
          ...
          for rr in range(NUM_ROBOTS):
            trans_inv[3*rr:3*rr+2, 3*rr+2] = self.J @ np.array((self.robot_system.xyt[3*rr], self.robot_system.xyt[3*rr+1]))
